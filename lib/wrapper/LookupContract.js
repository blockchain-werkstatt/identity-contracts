...
var LookupContract = function () {
  function LookupContract(lookupContractAddress, lightwallet, walletCrypto) {
    ...
    // SMART CONTRACTS
    this.lookupContract = this.lightWallet.createContractInstance({
  ...
  _createClass(LookupContract, [{
    ...
    value: function getIdentityAddressFromLookupContract(address) {
      ...
      console.log('LookupContract Address: ' + this.lookupContractAddress);
      console.log('ownerAddress: ' + address);
      return new Promise(function (resolve, reject) {
        _this.lookupContract.getIdentityAddress(address, function (err, result) {
          if (err) {
            reject(err);
          }
    ...
    value: function addIdentityAddressToLookupContract(_ref) {
      ...
      console.log('LookupContract: add identity address to Lookup Contract -> ' + this.lookupContractAddress);
      ...
      return this.lightWallet.contractMethodTransaction(this.lookupContract, methodName, args, pin).then(function (transactionHash) {
    ...
    value: function addIdentityAddressToLookupForOtherEntity(_ref2) {
      ...
      console.log('LookupContract ' + this.lookupContractAddress + ': add for owner: ' + ownerMainAddress + ' identityAddress:' + identityAddress);
